...
type ActionInferTypesLegacy struct {
	ActionBase
}

// NewActionInferTypesLegacy creates the pre-parity ActionInferTypes in the given group.
// This is the committed-type-seeded implementation retained for the production
// linear pipeline (bridge/decompile.go) and its tests. The faithful, C++-parity
// ActionInferTypes (registered on the tree mainloop) lives in action_infertypes.go.
func NewActionInferTypesLegacy(group string) *ActionInferTypesLegacy {
	a := &ActionInferTypesLegacy{}
...
func (a *ActionInferTypesLegacy) Clone(groups ActionGroupList) Action {
	if !a.MatchGroup(groups) {
		return nil
	}
	return NewActionInferTypesLegacy(a.GetGroup())
}
...
func (a *ActionInferTypesLegacy) Apply(data *Funcdata) int {
