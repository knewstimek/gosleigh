...
| obtain context | `ObtainContext()` / `DisassemblyCache.ObtainParserContext()` | `Sleigh::obtainContext()` / `DisassemblyCache::getParserContext()` | `simplified-safe` | same-address hit reuse, circular slot reuse, address reassignment 시 parse-state reset과 `N2Addr` invalidation이 들어갔다. **세션4(`8b23afa`): in-flight pin 추가** -- pcode 빌드 중인 컨텍스트 슬롯을 재사용에서 제외해 C++ `oneInstruction`의 pos 생존 계약(minimumreuse window가 암묵 보장)을 명시적으로 실현. eager inst_next2 해석이 같은 풀을 recycle해 naddr/ops를 오염시키던 명령어-삼킴 버그 근절(회귀 테스트 TestPinContextSurvivesReuseWrap). Go는 deterministic injection용 explicit `contexts` map을 같이 유지하고, original `minimumreuse/windowsize` guarantee 전체는 아직 아니다 |
...
| return | ActionReturnSplit / NodeSplit | blockaction.cc:2264 / funcdata_block.cc:845 | `ported` | 세션4 착지(`3dc1479`, 보존브랜치 `42522d9` 본착지) -- CloneBlockOps SSA 수술 + gatherReturnGotos. 하류 collapse 갭(isComplex compound + overflow while(true))도 함께 착지해 parse_steps 의미 무손상 |
| type-model | spacebase PTRSUB 출력 포인터 타이핑 | funcdata.cc:391-419 / typeop.cc:1217,2368 | `ported` | 세션4 착지(`9a64a8f`) -- 락된 spacebase 입력을 propagation source로 매 InferTypes 패스마다 출력 재유도(직접 락 아님). struct/array 내부 오프셋 downChain(TypePointerRel)은 TODO |
| structuring | BlockBasic::isComplex leaf | block.cc:2388 | `unimplemented` | 세션4 known gap 스텁(`40d00a3`) -- Gosleigh가 deadcode/MarkImplied를 print 시점으로 미뤄 pre-structure SSA 잔여 op에 faithful 통계가 과발화(6게이트 회귀). pre-structure SSA 정합 선행 필요 |
| param-recovery | arg 레지스터 sub-register 폭 / spurious 입력 | coreaction.cc ActionActiveParam / heritage | `mismatch` | reverse_bytes_inplace에서 spurious RDX(sz8)+phantom R8 입력 -> param 오바인딩 -> 캐리어 선언 소실 (세션4 진단, NEXT_SESSION_PROMPT (B)) |
