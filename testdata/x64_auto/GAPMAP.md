...
| `dowhile_count` | TEMP | TEMP: extra temp/local identifiers in output (3 vs 2): iVar1 |
...
| `reverse_bytes_inplace` | TYPECAST | TYPECAST: cast (longlong): want=0 got=2 |
...
| `char_arith_promote` | TYPECAST, PTR | TYPECAST: cast (int): want=1 got=0<br>PTR: raw pointer scale '* 2': want=0 got=1 |
...
| `nested_if_ladder_grade` | TEMP | TEMP: fewer temp/local identifiers than golden (0 vs 1), missing: uVar1 |
